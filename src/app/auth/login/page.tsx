...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-background-light dark:bg-background-dark relative overflow-hidden font-sans">
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-cyan-500/10 rounded-full blur-[120px] animate-float"></div>
      <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-violet-600/10 rounded-full blur-[120px] animate-float" style={{ animationDelay: '3s' }}></div>
      ...
      <div className="w-full max-w-md glass-card relative z-10 animate-slide-up border-cyan-500/10">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-display font-black text-slate-900 dark:text-white tracking-tight mb-2">
            Логістика <span className="bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">v2.0</span>
          </h1>
          <p className="text-slate-500 dark:text-slate-400 font-display font-bold text-xs uppercase tracking-[0.3em]">
            Elite Transport Ecosystem
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          <div className="space-y-3 animate-fade-in animate-delay-100">
            <label className="text-[10px] font-display font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">
              Ідентифікатор / Email
            </label>
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-14 pr-6 py-4.5 bg-white/50 dark:bg-black/20 border border-slate-200 dark:border-white/5 rounded-2xl text-slate-900 dark:text-white placeholder-slate-400/60 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500/50 transition-all duration-300 font-medium"
              />
            </div>
            {errors.email && (
          ...
          <div className="space-y-3 animate-fade-in animate-delay-200">
            ...
            {errors.password && (
              <p className="mt-2 text-[10px] font-display font-black text-rose-500 uppercase tracking-widest ml-1">{errors.password.message}</p>
            )}
          </div>
          ...
          <Button
            ...
            className="w-full !py-5 shadow-glow !rounded-2xl !bg-cyan-600 hover:!bg-cyan-500 transition-all active:scale-[0.98] font-display font-black tracking-[0.2em] text-[11px] uppercase text-white border-none relative overflow-hidden group animate-fade-in animate-delay-300"
          >
            <span className="relative z-10">{loginMutation.isPending ? 'Синхронізація...' : 'Увійти в систему'}</span>
        ...
        <div className="mt-12 pt-8 border-t border-slate-100 dark:border-white/5 animate-fade-in animate-delay-500">
          <p className="text-center text-[11px] font-display font-bold text-slate-500 uppercase tracking-widest">
            Потрібен доступ?{' '}
            <a href="/auth/register" className="text-cyan-500 hover:text-cyan-400 transition-colors">
              Зареєструватися
            </a>
          </p>
        </div>
